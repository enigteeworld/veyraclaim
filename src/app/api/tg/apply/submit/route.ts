// src/app/api/tg/apply/submit/route.ts
...
function verifyInitData(initData: string) {
  ...
  // Optional freshness: auth_date within 24h
...
export async function POST(req: Request) {
  try {
    ...
    // 1) Load session from form_sessions
    ...
    const { data: camp, error: campErr } = await supabaseAdmin
      .from("campaigns")
      .select("id,code,min_tier,type,max_slots")
    ...
    // 4) Load questions to validate required fields (uses YOUR schema)
    ...
    for (const q of qs || []) {
      const key = String((q as any).key || "");
      const label = String((q as any).label || "Question");
      const requiredQ = (q as any).required === false ? false : true;

      if (!key) continue;
      const v = String(answers?.[key] ?? "").trim();
      if (requiredQ && !v) {
        return NextResponse.json({ ok: false, error: `Please answer: ${label}` }, { status: 400 });
      }
    }

    // 5) Re-check eligibility server-side (authoritative)
    ...
    // Optional: enforce max slots (same logic as /join)
    if (camp.max_slots) {
      ...
      if ((count ?? 0) >= Number(camp.max_slots)) {
        return NextResponse.json({ ok: false, error: "This campaign is full (max slots reached)." }, { status: 400 });
      }
    }

    // 6) Insert application as campaign_entries (answers stored)
    const { error: insErr } = await supabaseAdmin.from("campaign_entries").insert({
      campaign_id: camp.id,
      telegram_user_id,
      wallet,
      tier: score.tier,
      ...
      proof_links: null,
      answers, // JSON object keyed by campaign_questions.key
    });

    if (insErr) {
      // common case: user already applied / unique constraint
      return NextResponse.json({ ok: false, error: `Couldn’t submit (maybe already submitted): ${insErr.message}` }, { status: 400 });
    }
